        ...
        if not node_prefix.startswith(str(connection_type)) and (offset_tobeadded==connection_type):        
            for var_name, var_value in item.items():
                if var_name not in [nodelayer]:
                    ...
                    print(f'inside loop register_name: {register_name}')
                    register_offset = item.get("Offset Address","")
                    print(f'inside loop register_offset: {register_offset}')
                    if (menu_name=="interrupt"):
                        #processor_instance = item.get("Processor instance","")
                        PLIC_number = item.get("PLIC","")
                        driver_pin = item.get("Driver pin","")
                        if PLIC_number is not None:
                            kconfig_lines.append(f'{indent*(depth-1)} config {processor_instance}->{driver_pin} ')
                    ...
                    break
                    #except: pass
                    #kconfig_lines.append(f'{indent*depth}{var_name}  {var_value}')
                #continue
            if nodelayer in item:
        ...
        if  node_prefix.startswith(str(connection_type)):
            for var_name, var_value in item.items():
                if var_name not in [nodelayer]:
                    ...
                    #try:
                    register_name = item.get("Node name","").split(":")[0]
                    print(f'inside 2nd loop register_name: {register_name}')
                    register_offset = item.get("Offset Address","")
                    print(f'inside 2nd loop register_offset: {register_offset}')
                    ...
                    if (menu_name=="interrupt"):
                        ...
                        PLIC_number = item.get("PLIC","")
                        driver_pin = item.get("Driver pin","")
                        print(f'PLIC_number: {PLIC_number}')
                        print(f'driver_pin: {driver_pin}')
                        if PLIC_number is not None:
                            kconfig_lines.append(f'{indent*(depth-1)} config {driver_pin} ')
                            kconfig_lines.append(f'{indent*(depth)} default "{PLIC_number}"  ')
                            break
                    #break
                    #except: pass
...
if __name__== "__main__":
    ...
    output_kconfig_file = pwd+"\\output.kconfig"
        ...
        for filename in files:
            for keyword in keyword_in_file:
                if keyword in filename.lower():
                    ...
                    #all_kconfig_sections[keyword]=[]
                    if categorise:                  
                        if node_prefix not in all_kconfig_sections:
                            all_kconfig_sections[node_prefix]=[]
                            #all_kconfig_sections[node_prefix].append(f'"{node_name[1:5]}"')
